...
# def rate_blur_in_folder(
#     folder_path, threshold=100.0, output_csv="bluriness_ratings.csv"
# ):
#     """
...
#         print(f"File {image_file}, Blurry: {blurry}, Variance: {laplacian_var}")
...
#             writer.writerow(
#                 {"Filename": image_file, "Laplacian Variance": laplacian_var}
#             )
...
def main():
    parser = argparse.ArgumentParser(
        description="Rate images in a folder for blurriness."
    )
    parser.add_argument(
        "folder_path", type=str, help="Path to the folder containing PNG images."
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=100.0,
        help="Blurriness threshold (default: 100.0). Lower values mean blurrier.",
    )
    parser.add_argument(
        "--output_csv",
        type=str,
        default="bluriness_ratings.csv",
        help="Output CSV file name (default: 'bluriness_ratings.csv').",
    )
